...
오늘은 휴가 정책 도메인을 Flex·근로기준법 기준으로 정밀화하고, 목업 데모를 "온보딩 체험"용으로 재편했습니다. 핵심은 (1) 법정/약정 휴가를 8대 법정휴가 기준으로 정확히 가르고, (2) 일부유급을 %가 아닌 유급 일수로 바로잡고, (3) 휴가 취소에 승인 워크플로우를 추가하고, (4) 목업 DB를 구성원만 남긴 깨끗한 신규 회사 상태로 비운 것입니다.

**[휴가 정책 정밀화]**
☑ 법정/약정 재분류 — 8대 법정휴가(연차·보상·생리·유산사산·출산전후·배우자출산·난임·가족돌봄) 기준, **보상휴가(§57) 법정 편입**·군소집훈련 복구·병가/경조사 약정
☑ 연차 §60 가산 검증(1년차 15→2년마다+1→최대25), 일부유급 **% → 유급 일수**(`partialPayDays`) — 출산전후 60/75일·난임 2일, 휴일차감·성별(배우자출산 남성)
☑ **휴가 취소 승인 워크플로우**(`approveOnCancel`) — 승인 완료 휴가 취소 시 승인자 취소승인, 반려 시 APPROVED 복귀, 참조자 알림 (`CANCEL_PENDING` 상태)
☑ 필수 휴가 삭제 차단 + **법령 가이드 패널**(권장설정 비교 "적합한 설정이에요") + "법정의무휴가 추가"·"기본 정책 만들기" 버튼

**[목업 온보딩 체험화]**
☑ 목업(DEMO-0000) = **구성원·조직·권한만** — 휴가종류·정책·공휴일·연차부여·신청·결재 전부 비움(실제 신규 회사처럼 체험자가 버튼으로 설정/축적)
☑ 선택 재시드(`reseed-mockup`, FK 의존순서 삭제·DEMO-0001 보존) · DEMO 배너로 인한 탑바 그리드 늘어남 버그 수정 · "전사"→"전체" 라벨 통일

**[UI 통일]**
☑ 페이지 헤더 이중 보더 제거·구성원 페이지 내부 스크롤 · 설정 페이지 DEMO 배너 · 로그인 체험 문구 정리
