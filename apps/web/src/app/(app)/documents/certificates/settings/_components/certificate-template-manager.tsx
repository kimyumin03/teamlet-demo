...
import { createCertificateTemplateAction, deleteCertificateTemplateAction } from "@/lib/actions/document";
import type { CertificateTemplateItem } from "@teamlet/modules/document";
      ...
      <div style={{ border: "1px solid var(--border)", borderRadius: 12, padding: "20px 24px" }}>
        ...
        <form onSubmit={handleAdd} style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: "2 1 280px", minWidth: 200 }}>
            <label style={{ fontSize: "12px", color: "var(--fg-muted)" }}>파일 URL</label>
            <Input
              required
              type="url"
              placeholder="https://..."
          ...
          <Button type="submit" disabled={isPending || !name.trim() || !fileUrl.trim()} style={{ flexShrink: 0 }}>
            {isPending ? "추가 중…" : "+ 추가"}
          </Button>
        </form>
        {addError && (
