...
function DeptCard({ node, emps }: { node: TreeNode; emps: EmployeeListItem[] }) {
  return (
    <div className="w-44 rounded-xl border border-border bg-background-primary p-3 shadow-sm">
      ...
      {emps.length === 0 ? (
        ...
        <div className="flex flex-col gap-1">
          ...
          {emps.length > 5 && (
            <p className="text-[10px] text-foreground-subtle">+{emps.length - 5}명 더</p>
          )}
        </div>
      )}
    </div>
...
function OrgNode({
  ...
  node: TreeNode;
  empsByDept: Map<string, EmployeeListItem[]>;
}) {
...
export function OrgTree({
  ...
  const empsByDept = new Map<string, EmployeeListItem[]>();
          ...
          <div className="mt-10 w-full border-t border-border pt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {unassigned.map((e) => (
                <div
                  key={e.id}
                  className="flex items-center gap-1.5 rounded-lg border border-border bg-background-primary px-2.5 py-1.5"
                >
                  <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-background-secondary text-[9px] font-medium text-foreground-muted">
                    {e.name.charAt(0)}
                  </div>
                  <div>
                    ...
                    {e.positionName && (
                      <p className="text-[10px] text-foreground-subtle">{e.positionName}</p>
                    )}
                  </div>
                </div>
              ))}
